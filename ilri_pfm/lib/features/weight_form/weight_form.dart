...
import 'package:ilri_pfm/common_widgets/number_input.dart';
...
class WeightForm extends StatefulWidget {
  ...
  const WeightForm({super.key, this.weight});
...
class _WeightFormState extends State<WeightForm> {
  ...
  void initState() {
    setState(() {
      _nameController.text = widget.weight?.weight.toString() ?? '';
    });
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
          ...
          child: Column(
            children: [
              ...
              CustomSwitch(
                  text: 'Active',
                  value:
                      widget.weight != null ? widget.weight?.is_active : true,
        ...
        Visibility(
          visible: widget.weight != null ? true : false,
          child: ContainerCard(
              child: Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              widget.weight?.is_active == true
                  ? DeactivateIcon(
  ...
  void patch() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (_nameController.text != widget.weight?.weight) {
        patchData['name'] = _nameController.text;
      }
      if (_isActive != widget.weight?.is_active) {
      ...
      if (patchData.isEmpty != true) {
        try {
          final LayedPlace? farm = await _repository.patch(
              id: widget.weight?.id ?? 0, data: patchData);
          _responseMessage(farm);
        } catch (e) {
          _errorMessage();
        }
  ...
  void activate() async {
    try {
      final LayedPlace? result = await _repository.updateState(
          id: widget.weight?.id ?? 0, state: true);
  ...
  void deActivate() async {
    try {
      final LayedPlace? result = await _repository.updateState(
          id: widget.weight?.id ?? 0, state: false);
      _responseMessage(result);
    } catch (e) {
      _errorMessage();
    }
