...
import { FlocksList, FlockService } from "@/features/flocks";
...
const Actions = (): ReactElement => {
  ...
  const handleExport = async (type: string) => {
    try {
      ...
      if (type == "xlsx") response = await FlockService.export.xlsx();
      if (type == "xls") response = await FlockService.export.xls();
      if (type == "csv") response = await FlockService.export.csv();
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await FlockService.import.xlsx(formData);
        if (file.name.includes(".xls"))
          response = await FlockService.import.xls(formData);
        if (file.name.includes(".csv"))
          response = await FlockService.import.csv(formData);
        if (response.status == 200) {
