...
import { EggsList, EggService } from "@/features/eggs";
...
const EggPage = () => {
  ...
  return (
    <ListLayout
      ...
      header={<Typography variant="title">Egg Productions</Typography>}
      actions={<Actions />}
    >
      <EggsList />
    </ListLayout>
...
const Actions = (): ReactElement => {
  ...
  const handleExport = async (type: string) => {
    try {
      ...
      if (type == "xlsx") response = await EggService.export.xlsx();
      if (type == "xls") response = await EggService.export.xls();
      if (type == "csv") response = await EggService.export.csv();
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await EggService.import.xlsx(formData);
        if (file.name.includes(".xls"))
          response = await EggService.import.xls(formData);
        if (file.name.includes(".csv"))
          response = await EggService.import.csv(formData);
        if (response.status == 200) {
