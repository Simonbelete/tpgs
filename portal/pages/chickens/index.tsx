...
import { ChickenList, ChickenService } from "@/features/chickens";
...
const Actions = (): ReactElement => {
  ...
  const handleExport = async (type: string) => {
    try {
      ...
      if (type == "xlsx") response = await ChickenService.export.xlsx();
      if (type == "xls") response = await ChickenService.export.xls();
      if (type == "csv") response = await ChickenService.export.csv();
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await ChickenService.import.xlsx(formData);
        if (file.name.includes(".xls"))
          response = await ChickenService.import.xls(formData);
        if (file.name.includes(".csv"))
          response = await ChickenService.import.csv(formData);
        if (response.status == 200) {
