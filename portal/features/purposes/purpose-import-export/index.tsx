...
const PurposeImportExport = () => {
  const { enqueueSnackbar, closeSnackbar } = useSnackbar();

  const handleExport = async (type: string) => {
    try {
      let response: Partial<AxiosResponse> = {};
      if (type == "xlsx") response = await exportPurposesXLSX ();
      if (type == "xls") response = await exportPurposesXLS();
      if (type == "csv") response = await exportPurposesCSV();
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await importPurposesXLSX(formData);
        if (file.name.includes(".xls"))
          response = await importPurposesXLS(formData);
        if (file.name.includes(".csv"))
          response = await importPurposesCSV(formData);
        if (response.status == 200) {
...
export default PurposeImportExport;
