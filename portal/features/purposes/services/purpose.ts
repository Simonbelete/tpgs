...
import { AbstractSummary, Response, Purpose, PurposeHistory } from "@/models";
import { AxiosResponse } from "axios";
import clientSSR from "@/services/client_ssr";
import client from "@/services/client";
import { NextPageContext } from "next";

export const URL = "/purposes";
...
export const purposeApi = baseApi.injectEndpoints({
  endpoints: (build) => {
    return {
      getPurposes: build.query<Response<Purpose[]>, Object>({
        query: (query?: Object) => ({
          url: `${URL}/`,
          method: "get",
          params: query,
      ...
      getPurposeHistory: build.query<
        Response<PurposeHistory[]>,
      ...
      getPurposeSummary: build.query<AbstractSummary, number>({
        query: (id: number) => ({
          url: `${URL}/${id}/${SUMMARY_URL}/`,
          method: "get",
        }),
      }),
      createPurpose: build.mutation<Promise<Purpose>, Partial<Purpose>>({
        query: (data: Partial<Purpose>) => ({
          url: `${URL}/`,
          method: "post",
          data: data,
        }),
      ...
      updatePurpose: build.mutation<
        Promise<Purpose>,
        Pick<Purpose, "id"> & Partial<Purpose>
      >({
        query: ({ id, ...patch }) => ({
          url: `${URL}/${id}/`,
          method: "patch",
...
export const exportPurposesCSV = async () =>
