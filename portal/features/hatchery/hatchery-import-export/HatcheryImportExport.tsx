...
const HatcheryImportExport = () => {
  const { enqueueSnackbar, closeSnackbar } = useSnackbar();

  const handleExport = async (type: string) => {
    try {
      let response: Partial<AxiosResponse> = {};
      if (type == "xlsx") response = await exportHatcheryXLSX();
      if (type == "xls") response = await exportHatcheryXLS();
      if (type == "csv") response = await exportHatcheryCSV();
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await importHatcheryXLSX(formData);
        if (file.name.includes(".xls"))
          response = await importHatcheryXLS(formData);
        if (file.name.includes(".csv"))
          response = await importHatcheryCSV(formData);
        if (response.status == 200) {
...
export default HatcheryImportExport;
