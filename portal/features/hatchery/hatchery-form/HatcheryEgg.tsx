...
import { HatcheryEgg, Nutrient } from "@/models";
...
const EditToolbar = (props: {
  ...
  const rows: GridRowsProp<
    Partial<HatcheryEgg> & Partial<{ isNew?: boolean }>
...
const HatcheryEggs = ({ id }: { id?: number }) => {
  const dispatch = useDispatch();
  const rows: GridRowsProp<
    Partial<HatcheryEgg> & Partial<{ isNew?: boolean }>
  > = useSelector((state: RootState) => state.ingredientForm.nutrients);
  ...
  const [createHatcheryEgg, createResult] = useCreateHatcheryEggMutation();
  const [updateHatcheryEgg, updateResult] =
    useUpdateIngredientNutrietMutation();
  const [deleteHatcheryEgg, deleteResult] = useDeleteHatcheryEggMutation();
  ...
  const processRowUpdate = async (
    ...
    const bodyData: Partial<HatcheryEgg> = {
  ...
  const onCreate = async (row: GridRowModel, data: Partial<HatcheryEgg>) => {
    if (id == null) return;

    const response = await createHatcheryEgg({ id, data }).unwrap();
    trigger({ id, query: {} }, true);
  };

  const onUpdate = async (newRow: GridRowModel, data: Partial<HatcheryEgg>) => {
    if (id == null) return;

    const response = await updateHatcheryEgg({ ...data, ingredient: id });
    trigger({ id, query: {} }, true);
  };

  const handleDelete = async (row_id: string | number) => {
    if (id == null) return;
    const response = await deleteHatcheryEgg({
      id,
