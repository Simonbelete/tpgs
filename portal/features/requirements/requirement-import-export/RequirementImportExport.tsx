...
const RequirementImportExport = () => {
  const { enqueueSnackbar, closeSnackbar } = useSnackbar();

  const handleExport = async (type: string) => {
    try {
      let response: Partial<AxiosResponse> = {};
      if (type == "xlsx") response = await exportRequirementsXLSX();
      if (type == "xls") response = await exportRequirementsXLS();
      if (type == "csv") response = await exportRequirementsCSV();
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await importRequirementsXLSX(formData);
        if (file.name.includes(".xls"))
          response = await importRequirementsXLS(formData);
        if (file.name.includes(".csv"))
          response = await importRequirementsCSV(formData);
        if (response.status == 200) {
...
export default RequirementImportExport;
