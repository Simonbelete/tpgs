...
const FormulaImportExport = () => {
  const { enqueueSnackbar, closeSnackbar } = useSnackbar();

  const handleExport = async (type: string) => {
    try {
      let response: Partial<AxiosResponse> = {};
      if (type == "xlsx") response = await exportFormulasXLSX ();
      if (type == "xls") response = await exportFormulasXLS();
      if (type == "csv") response = await exportFormulasCSV();
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await importFormulasXLSX(formData);
        if (file.name.includes(".xls"))
          response = await importFormulasXLS(formData);
        if (file.name.includes(".csv"))
          response = await importFormulasCSV(formData);
        if (response.status == 200) {
...
export default FormulaImportExport;
