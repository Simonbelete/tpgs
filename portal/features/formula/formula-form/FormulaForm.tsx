...
const FormulaForm = ({
  ...
  const [formulateTrigger, { isLoading: formulateIsLoading, data: formulated}] = useLazyFormulateQuery();
  ...
  useEffect(() => {
    if(formula !== undefined){
      formulaIngredientTrigger({id: formula?.id, query: {}}).then((response) => {
  ...
  useEffect(() => {
    const cost_achived: number = (formulated == null ? formula?.ration_price : formulated.results.ration_price) || 0
    const cost_goal: number = (formulated == null ? formula?.budget : formulated.results.budget) || 0

    const ratio_achived: number = (formulated == null ? formula?.ration_ratio : formulated.results.ration_ratio) || 0
    const ratio_goal: number = (formulated == null ? formula?.desired_ratio : formulated.results.desired_ratio) || 0

    const weight_achived: number = (formulated == null ? formula?.ration_weight : formulated.results.ration_weight) || 0
    const weight_goal: number = (formulated == null ? formula?.weight : formulated.results.weight) || 0

    const dm_achived: number = (formulated == null ? formula?.ration_dm : formulated.results.ration_dm) || 0
    const dm_goal: number = (formulated == null ? formula?.desired_dm : formulated.results.desired_dm) || 0

    setGoalAchivements({
      total_cost: Number((cost_achived / (cost_goal == 0 ? 1: cost_goal) * 100).toFixed(3)),
      total_ratio: Number((ratio_achived / (ratio_goal == 0 ? 1: ratio_goal) * 100).toFixed(3)),
      total_weight: Number((weight_achived / (weight_goal == 0 ? 1: weight_goal) * 100).toFixed(3)),
      total_dm: Number((dm_achived / (dm_goal == 0 ? 1: dm_goal) * 100).toFixed(3)),
    })
  }, [formula, formulated]);
  ...
  return (
    <>
              ...
              <Stack>
                ...
                <Stack direction={"row"} spacing={2}>
                  <Tooltip title="Achived Cost">
                    <Typography variant="body2" fontWeight={600} color="secondary">{formulated == null ? formula?.ration_price : formulated.results.ration_price}</Typography>
                  </Tooltip>
                ...
                <Tooltip title="Desired Cost">
                  <Typography variant="caption" color="text.secondary">{formulated == null ? formula?.budget : formulated.results.budget}</Typography>
                </Tooltip>
              </Stack>
              ...
              <Stack>
                ...
                <Stack direction={"row"} spacing={2}>
                  <Tooltip title="Achived Ration">
                    <Typography variant="body2" fontWeight={600} color="secondary">{formulated == null ? formula?.ration_ratio : formulated.results.ration_ratio}</Typography>
                  </Tooltip>
                  <Tooltip title="Achivement">
                ...
                <Tooltip title="Desired Cost">
                  <Typography variant="caption" color="text.secondary">{formulated == null ? formula?.desired_ratio : formulated.results.desired_ratio}</Typography>
                </Tooltip>
              </Stack>
              ...
              <Stack>
                ...
                <Stack direction={"row"} spacing={2}>
                  <Tooltip title="Achived Weight">
                    <Typography variant="body2" fontWeight={600} color="secondary">{formulated == null ? formula?.ration_weight : formulated.results.ration_weight}</Typography>
                  </Tooltip>
                  <Tooltip title="Achivement">
                ...
                <Tooltip title="Desired Cost">
                  <Typography variant="caption" color="text.secondary">{formulated == null ? formula?.weight : formulated.results.weight}</Typography>
                </Tooltip>
              </Stack>
              ...
              <Stack>
                ...
                <Stack direction={"row"} spacing={2}>
                  <Tooltip title="Achived DM">
                    <Typography variant="body2" fontWeight={600} color="secondary">{formulated == null ? formula?.ration_dm : formulated.results.ration_dm}</Typography>
                  </Tooltip>
                  <Tooltip title="Achivement">
                ...
                <Tooltip title="Desired Cost">
                  <Typography variant="caption" color="text.secondary">{formulated == null ? formula?.desired_dm : formulated.results.desired_dm}</Typography>
                </Tooltip>
              </Stack>
      ...
      <Tabs value={tabIndex} onChange={handleTabChange}>
        ...
        <Tab
          label="Ingredients"
          icon={<Chip label={formulaState.ingredients.length || 0} size="small" />}
          iconPosition="end"
          {...a11yProps(1)}
        />
        <Tab
          label="Requirements"
          icon={<Chip label={formulaState.requirements.length || 0} size="small" />}
          iconPosition="end"
          {...a11yProps(2)}
        />
      </Tabs>
