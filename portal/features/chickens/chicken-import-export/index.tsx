  ...
  const handleExport = async (type: string) => {
    try {
      ...
      if (response.status == 200) {
        fileDownload(response.data, `chickens_.${type}`);
      } else {
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await importChickensXLSX(formData);
        if (file.name.includes(".xls"))
          response = await importChickensXLS(formData);
        if (file.name.includes(".csv"))
          response = await importChickensCSV(formData);
        if (response.status == 200) {
...
export default ChickenImportExport;
