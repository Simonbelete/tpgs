...
import { AbstractSummary, Response, Egg } from '@/models';
...
export const eggApi = baseApi.injectEndpoints({
  endpoints: (build) => {
    return {
      getEggs: build.query<Response<Egg>, Object>({ query: (query?: Object) => ({ url: `${URL}/`, method: 'get', params: query }) }),
      getEggHistory: build.query<Response<Egg>, {id: number, query: Object}>({ query: ({id, query}) => ({ url: `${URL}/${id}/${HISTORY_URL}`, method: 'get', params: query }) }),
      getEggSummary: build.query<AbstractSummary, number>({ query: (id: number) => ({ url: `${URL}/${id}/${SUMMARY_URL}/`, method: 'get' }) }),
      createEgg: build.mutation<Promise<AxiosResponse<Egg>>, Partial<Egg>>({
        query: (data: Partial<Egg>) => ({ url: `${URL}/`, method: 'post', data: data }),
      }),
      updateEgg: build.mutation<Egg, Pick<Egg, 'id'> & Partial<Egg>>({
...
export const exportEggsCSV = async () => client.get(`${EXPORT_URL}/csv`, { responseType: "blob" });
