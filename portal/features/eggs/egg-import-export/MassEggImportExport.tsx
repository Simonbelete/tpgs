...
const MassEggImportExport = () => {
  const { enqueueSnackbar, closeSnackbar } = useSnackbar();

  const handleExport = async (type: string) => {
    try {
      let response: Partial<AxiosResponse> = {};
      if (type == "xlsx") response = await exportEggsXLSX ();
      if (type == "xls") response = await exportEggsXLS();
      if (type == "csv") response = await exportEggsCSV();
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await importEggsXLSX(formData);
        if (file.name.includes(".xls"))
          response = await importEggsXLS(formData);
        if (file.name.includes(".csv"))
          response = await importEggsCSV(formData);
        if (response.status == 200) {
...
export default MassEggImportExport;
