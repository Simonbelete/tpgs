...
import { AbstractSummary, Response, Weight, WeightHistory } from "@/models";
import { AxiosResponse } from "axios";
import clientSSR from "@/services/client_ssr";
import client from "@/services/client";
import { NextPageContext } from "next";

export const URL = "/weights";
...
export const weightApi = baseApi.injectEndpoints({
  endpoints: (build) => {
    return {
      getWeights: build.query<Response<Weight[]>, Object>({
        query: (query?: Object) => ({
          url: `${URL}/`,
          method: "get",
          params: query,
      ...
      getWeightSummary: build.query<AbstractSummary, number>({
        query: (id: number) => ({
          url: `${URL}/${id}/${SUMMARY_URL}/`,
          method: "get",
        }),
      }),
      createWeight: build.mutation<Promise<Weight>, Partial<Weight>>({
        query: (data: Partial<Weight>) => ({
          url: `${URL}/`,
          method: "post",
          data: data,
        }),
...
export const exportWeightsCSV = async () =>
