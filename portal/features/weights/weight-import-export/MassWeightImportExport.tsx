...
const MassWeightImportExport = () => {
  const { enqueueSnackbar, closeSnackbar } = useSnackbar();

  const handleExport = async (type: string) => {
    try {
      let response: Partial<AxiosResponse> = {};
      if (type == "xlsx") response = await exportWeightsXLSX ();
      if (type == "xls") response = await exportWeightsXLS();
      if (type == "csv") response = await exportWeightsCSV();
      if (response.status == 200) {
        fileDownload(response.data, `weights_.${type}`);
      } else {
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await importWeightsXLSX(formData);
        if (file.name.includes(".xls"))
          response = await importWeightsXLS(formData);
        if (file.name.includes(".csv"))
          response = await importWeightsCSV(formData);
        if (response.status == 200) {
...
export default MassWeightImportExport;
