    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await importIngredientTypesXLSX(formData);
        if (file.name.includes(".xls"))
          response = await importIngredientTypesXLS(formData);
        if (file.name.includes(".csv"))
          response = await importIngredientTypesCSV(formData);
        if (response.status == 200) {
...
export default IngredientTypeImportExport;
