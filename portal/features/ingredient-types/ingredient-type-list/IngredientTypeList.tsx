...
import { useGetIngredientTypesQuery, useDeleteIngredientTypeMutation } from "../services";
...
const IngredientTypeList = () => {
  ...
  const { data, isLoading, refetch } = useGetIngredientTypesQuery(buildQuery({...buildPage(paginationModel), ...selector})); 
  const [deleteIngredientType, deleteResult ] = useDeleteIngredientTypeMutation();

  const handleDelete = async (id: number) => await deleteIngredientType(id).then(() => refetch())
