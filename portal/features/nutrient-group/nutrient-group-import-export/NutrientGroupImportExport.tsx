...
const NutrientGroupImportExport = () => {
  const { enqueueSnackbar, closeSnackbar } = useSnackbar();

  const handleExport = async (type: string) => {
    try {
      let response: Partial<AxiosResponse> = {};
      if (type == "xlsx") response = await exportNutrientGroupsXLSX ();
      if (type == "xls") response = await exportNutrientGroupsXLS();
      if (type == "csv") response = await exportNutrientGroupsCSV();
      if (response.status == 200) {
        fileDownload(response.data, `NutrientGroups_.${type}`);
      } else {
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await importNutrientGroupsXLSX(formData);
        if (file.name.includes(".xls"))
          response = await importNutrientGroupsXLS(formData);
        if (file.name.includes(".csv"))
          response = await importNutrientGroupsCSV(formData);
        if (response.status == 200) {
...
export default NutrientGroupImportExport;
