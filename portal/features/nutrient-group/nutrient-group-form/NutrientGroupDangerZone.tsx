...
import { useDeleteNutrientGroupMutation, useUpdateNutrientGroupMutation } from '../services';
import { useRouter } from 'next/router';
import { useCRUD } from "@/hooks";

const NutrientGroupDangerZone = ({ id, is_active }: { id: number, is_active: boolean }) => {
  ...
  const handleToggleActive = async (value: boolean) =>  await updateNutrientGroup({id: id, is_active: value})
  const handleDelete = async () => await deleteNutrientGroup(id).then(() => router.push('/nutrient-groups'))
