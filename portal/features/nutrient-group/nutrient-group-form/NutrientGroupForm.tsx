...
const NutrientGroupForm = ({
  ...
  const onSubmit: SubmitHandler<Inputs> = async (data) => {
    if (nutrientGroup == null)
      await createNutrientGroup(data).then(
        () => redirect && router.push("/nutrient-groups")
      );
  ...
  return (
    <>
      <Grid container spacing={4}>
        <Grid item xs={9}>
          <Card title="Nutrient Group Form">
          ...
          <Stack spacing={3}>
            {nutrientGroup && (
              <>
                ...
                <NutrientGroupDangerZone
                  id={nutrientGroup.id}
                  is_active={nutrientGroup.is_active}
                />
              </>
            )}
          </Stack>
