...
import { AbstractSummary, Response, NutrientGroup } from '@/models';
  ...
  endpoints: (build) => {
    return {
      ...
      createNutrientGroup: build.mutation<Promise<AxiosResponse<NutrientGroup>>, Partial<NutrientGroup>>({
        query: (data: Partial<NutrientGroup>) => ({ url: `${URL}/`, method: 'post', data: data }),
      }),
      updateNutrientGroup: build.mutation<NutrientGroup, Pick<NutrientGroup, 'id'> & Partial<NutrientGroup>>({
...
export const exportNutrientGroupsXLSX = async () => client.get(`${EXPORT_URL}/xlsx`);
export const exportNutrientGroupsXLS = async () => client.get(`${EXPORT_URL}/xls`);
export const exportNutrientGroupsCSV = async () => client.get(`${EXPORT_URL}/csv`, { responseType: "blob" });
