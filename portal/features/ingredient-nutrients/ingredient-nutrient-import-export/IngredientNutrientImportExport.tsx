...
const IngredientNutrientImportExport = () => {
  ...
  const handleExport = async (type: string) => {
    try {
      ...
      if (type == "xlsx") response = await exportIngredientNutrientsXLSX();
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await importIngredientNutrientsXLSX(formData);
        if (file.name.includes(".xls"))
          response = await importIngredientNutrientsXLS(formData);
...
export default IngredientNutrientImportExport;
