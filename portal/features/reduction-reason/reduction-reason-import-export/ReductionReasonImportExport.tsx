...
const ReductionReasonImportExport = () => {
  const { enqueueSnackbar, closeSnackbar } = useSnackbar();

  const handleExport = async (type: string) => {
    try {
      let response: Partial<AxiosResponse> = {};
      if (type == "xlsx") response = await exportReductionReasonsXLSX();
      if (type == "xls") response = await exportReductionReasonsXLS();
      if (type == "csv") response = await exportReductionReasonsCSV();
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await importReductionReasonsXLSX(formData);
        if (file.name.includes(".xls"))
          response = await importReductionReasonsXLS(formData);
        if (file.name.includes(".csv"))
          response = await importReductionReasonsCSV(formData);
        if (response.status == 200) {
  ...
  return (
    <>
      <Stack
        ...
        flexWrap="wrap"
      >
        <Link href="/reduction-reasons/create">
...
export default ReductionReasonImportExport;
