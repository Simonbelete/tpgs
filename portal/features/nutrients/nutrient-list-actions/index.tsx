...
const NutrientListActions = () => {
  ...
  const handleExport = async (type: string) => {
    try {
      ...
      nutrient_service;
      if (type == "xlsx") response = await nutrient_service.export.xlsx();
      if (type == "xls") response = await nutrient_service.export.xls();
      if (type == "csv") response = await nutrient_service.export.csv();
    ...
    if (event.target != null && event.target.files != null) {
      ...
      try {
        ...
        if (file.name.includes(".xlsx"))
          response = await nutrient_service.import.xlsx(formData);
        if (file.name.includes(".xls"))
          response = await nutrient_service.import.xls(formData);
        if (file.name.includes(".csv"))
          response = await nutrient_service.import.csv(formData);
        if (response.status == 200) {
...
export default NutrientListActions;
